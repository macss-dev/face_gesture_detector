...
void main() {
  group('PoseAngles', () {
    ...
    test('isFrontal returns true when yaw and pitch are within ±15°', () {
      expect(PoseAngles(pitch: 0.0, yaw: 0.0, roll: 0.0).isFrontal, isTrue);
      expect(
    ...
    test('isFrontal returns false when yaw exceeds ±15°', () {
      expect(PoseAngles(pitch: 0.0, yaw: 15.1, roll: 0.0).isFrontal, isFalse);
      expect(PoseAngles(pitch: 0.0, yaw: -16.0, roll: 0.0).isFrontal, isFalse);
    });

    test('isFrontal returns false when pitch exceeds ±15°', () {
      expect(PoseAngles(pitch: 15.1, yaw: 0.0, roll: 0.0).isFrontal, isFalse);
      expect(PoseAngles(pitch: -20.0, yaw: 5.0, roll: 0.0).isFrontal, isFalse);
    });

    test('isFrontal boundary: exactly ±15° is not frontal', () {
      // Strict less-than per architecture: yaw.abs() < 15.0
      expect(PoseAngles(pitch: 15.0, yaw: 0.0, roll: 0.0).isFrontal, isFalse);
      expect(PoseAngles(pitch: 0.0, yaw: 15.0, roll: 0.0).isFrontal, isFalse);
    });
  });
