...
void main() {
  group('HeadNodRecognizer', () {
    ...
    test('fires up nod when pitch exceeds +threshold for sustained duration', () {
      recognizer.addFaceFrame(_frame(timestamp: Duration.zero, pitch: 25));
      recognizer.addFaceFrame(_frame(timestamp: Duration(milliseconds: 200), pitch: 22));
      expect(events, isEmpty);

      recognizer.addFaceFrame(_frame(timestamp: Duration(milliseconds: 400), pitch: 28));
      expect(events, hasLength(1));
      expect(events.first.direction, HeadNodDirection.up);
      expect(events.first.sustainedFor, Duration(milliseconds: 400));
    });

    test('fires down nod when pitch exceeds -threshold for sustained duration', () {
      recognizer.addFaceFrame(_frame(timestamp: Duration.zero, pitch: -25));
      recognizer.addFaceFrame(_frame(timestamp: Duration(milliseconds: 400), pitch: -22));
    ...
    test('does not fire when pitch is within threshold', () {
      ...
      recognizer.addFaceFrame(_frame(timestamp: Duration(milliseconds: 400), pitch: 12));
    ...
    test('resets timer when pitch drops back within threshold', () {
      recognizer.addFaceFrame(_frame(timestamp: Duration.zero, pitch: 25));
      recognizer.addFaceFrame(_frame(timestamp: Duration(milliseconds: 200), pitch: 25));

      // Returns to center
      recognizer.addFaceFrame(_frame(timestamp: Duration(milliseconds: 300), pitch: 5));

      // Nods again — timer restarts
      recognizer.addFaceFrame(_frame(timestamp: Duration(milliseconds: 400), pitch: 25));
      recognizer.addFaceFrame(_frame(timestamp: Duration(milliseconds: 600), pitch: 25));
      expect(events, isEmpty);

      recognizer.addFaceFrame(_frame(timestamp: Duration(milliseconds: 800), pitch: 25));
      expect(events, hasLength(1));
    });

    test('reports pitch angle of triggering frame', () {
      recognizer.addFaceFrame(_frame(timestamp: Duration.zero, pitch: 30));
      recognizer.addFaceFrame(_frame(timestamp: Duration(milliseconds: 400), pitch: 35));
    ...
    test('reset clears sustained tracking', () {
      ...
      recognizer.addFaceFrame(_frame(timestamp: Duration(milliseconds: 300), pitch: 25));
      expect(events, isEmpty);
    });
  });
}
