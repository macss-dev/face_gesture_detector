...
void main() {
  group('MouthRecognizer', () {
    ...
    test('fires when jawOpen exceeds threshold for sustained duration', () {
      ...
      recognizer.addFaceFrame(_frame(timestamp: Duration(milliseconds: 200), jawOpen: 0.25));
      expect(events, isEmpty);

      recognizer.addFaceFrame(_frame(timestamp: Duration(milliseconds: 400), jawOpen: 0.35));
    ...
    test('does not fire when below threshold', () {
      ...
      recognizer.addFaceFrame(_frame(timestamp: Duration(milliseconds: 400), jawOpen: 0.05));
    ...
    test('resets timer when value drops below threshold', () {
      recognizer.addFaceFrame(_frame(timestamp: Duration.zero, jawOpen: 0.3));
      recognizer.addFaceFrame(_frame(timestamp: Duration(milliseconds: 200), jawOpen: 0.3));
      recognizer.addFaceFrame(_frame(timestamp: Duration(milliseconds: 300), jawOpen: 0.05));
      recognizer.addFaceFrame(_frame(timestamp: Duration(milliseconds: 400), jawOpen: 0.3));
      recognizer.addFaceFrame(_frame(timestamp: Duration(milliseconds: 600), jawOpen: 0.3));
      expect(events, isEmpty);

      recognizer.addFaceFrame(_frame(timestamp: Duration(milliseconds: 800), jawOpen: 0.3));
      expect(events, hasLength(1));
    });
    ...
    test('reset clears sustained tracking', () {
      ...
      recognizer.addFaceFrame(_frame(timestamp: Duration(milliseconds: 300), jawOpen: 0.3));
      expect(events, isEmpty);
    });
  });
}
