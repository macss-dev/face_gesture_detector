...
FaceFrame _frame({required Duration timestamp, double yaw = 0.0}) {
  return FaceFrame(
    timestamp: timestamp,
    isFaceDetected: true,
    faceConfidence: 0.9,
...
void main() {
  group('HeadTurnRecognizer', () {
    ...
    test(
      'fires left turn when yaw exceeds +threshold for sustained duration',
      () {
        recognizer.addFaceFrame(_frame(timestamp: Duration.zero, yaw: 30));
        recognizer.addFaceFrame(
          _frame(timestamp: Duration(milliseconds: 200), yaw: 28),
        );
        expect(events, isEmpty);

        recognizer.addFaceFrame(
          _frame(timestamp: Duration(milliseconds: 400), yaw: 32),
        );
    ...
    test(
      'fires right turn when yaw exceeds -threshold for sustained duration',
      () {
        recognizer.addFaceFrame(_frame(timestamp: Duration.zero, yaw: -30));
        recognizer.addFaceFrame(
          _frame(timestamp: Duration(milliseconds: 400), yaw: -28),
        );
    ...
    test('does not fire when yaw is within threshold', () {
      ...
      recognizer.addFaceFrame(
        _frame(timestamp: Duration(milliseconds: 400), yaw: 12),
      );
    ...
    test('resets timer when yaw drops back within threshold', () {
      recognizer.addFaceFrame(_frame(timestamp: Duration.zero, yaw: 30));
      recognizer.addFaceFrame(
        _frame(timestamp: Duration(milliseconds: 200), yaw: 30),
      );

      // Returns to center
      recognizer.addFaceFrame(
        _frame(timestamp: Duration(milliseconds: 300), yaw: 5),
      );

      // Turns again — timer restarts
      recognizer.addFaceFrame(
        _frame(timestamp: Duration(milliseconds: 400), yaw: 30),
      );
      recognizer.addFaceFrame(
        _frame(timestamp: Duration(milliseconds: 600), yaw: 30),
      );
      expect(events, isEmpty);

      recognizer.addFaceFrame(
        _frame(timestamp: Duration(milliseconds: 800), yaw: 30),
      );
      expect(events, hasLength(1));
    });
    ...
    test('reports yaw angle of triggering frame', () {
      ...
      recognizer.addFaceFrame(
        _frame(timestamp: Duration(milliseconds: 400), yaw: 40),
      );
    ...
    test('reset clears sustained tracking', () {
      ...
      recognizer.addFaceFrame(
        _frame(timestamp: Duration(milliseconds: 300), yaw: 30),
      );
      expect(events, isEmpty);
    });
  });
